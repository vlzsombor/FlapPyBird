...
from src.neat.autoPlayer import Genome
from src.utils import GameConfig

filename = 'best_pickle.pkl'

class Population:
    def __init__(self, config: GameConfig, gh: GeneHistory, pop_size: int = 100, testAi = False) -> None: 
        self.best_fitness = 0
        ...
        if testAi:
            ...
            self.population.append(AutoPlayer(config, gh, brain=genome))
            return
    ...
    def reset(self):
        if self.testAi:
            ...
            self.population.append(AutoPlayer(self.config, self.gh, brain=genome))
            return
...
class PopulationPong:
    def __init__(self, gh: GeneHistory, pop_size: int = 100, testAi = False) -> None: 
        self.best_fitness = 0
        self.pop_size = pop_size  # Size of population
